...
const EMAIL_SUBJECT = 'Verify your email for our Challenge3!';
const EMAIL_BODY = 'Thanks for signing up to our Challenge3! Your verification code is {####}';
const CUSTOM_ATTRIBUTES = {
  id: new StringAttribute({ mutable: true }),
};
...
export class IdPool extends UserPool {
  ...
  constructor(scope: Construct, id: string, prorps: Challenge1UserPoolProps) {
    ...
    this.addPostConfirmationTrigger(id);
  }
  ...
  private addPostConfirmationTrigger(id: string) {
    const trigger = this.createTriggerFunction(`${id}PostConfirmationTrigger`, 'user.ts', 10);

    const policy = new Policy(this, 'PostConfirmationTriggerPolicy', {
      statements: [
        new PolicyStatement({
    ...
    trigger.role?.attachInlinePolicy(policy);
    this.addTrigger(UserPoolOperation.POST_CONFIRMATION, trigger);
  }
